...
pub fn comorbidity_of_given_mental_health_condition(condition: MentalHealthCondition, gender: Gender, adhd_type: AdhdSubtype, algo: MLAlgorithms, split_ratio: f32) -> MLResponse {
    ...
    let df = get_all_patient_info_raw(true)
        ...
        .select([col("SEX"), col("AGE"), col("ADHD"), col("ADD"), col("TARGET")])
        .collect()?;
...
mod test {
    ...
    fn trains_model_for_comorbidity_of_bipolar_disorder() {
        let result = comorbidity_of_given_mental_health_condition(MentalHealthCondition::BipolarDisorder, 
                                                                  Gender::None, 
                                                                  AdhdSubtype::All, 
    ...
    fn trains_model_for_comorbidity_of_unipolar_depression() {
        let result = comorbidity_of_given_mental_health_condition(MentalHealthCondition::UnipolarDepression,
                                                                  Gender::None,
                                                                  AdhdSubtype::All,
                                                                  MLAlgorithms::LogisticRegression,
                                                                  0.70).unwrap();        
    ...
    fn trains_model_for_comorbidity_of_anxiety() {
        let result = comorbidity_of_given_mental_health_condition(MentalHealthCondition::AnxietyDisorder,
                                                                  Gender::None,
                                                                  AdhdSubtype::All,
                                                                  MLAlgorithms::LogisticRegression,
                                                                  0.70).unwrap();            
    ...
    fn trains_model_for_comorbidity_of_substance_abuse_disorder() {
        let result = comorbidity_of_given_mental_health_condition(MentalHealthCondition::SubstanceAbuseDisorder,
                                                                  Gender::None,
                                                                  AdhdSubtype::All,
                                                                  MLAlgorithms::LogisticRegression,
                                                                  0.70).unwrap();            
